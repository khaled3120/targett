...
const TRANSLATIONS = {
  us: {
    ...
    header: "Target Voucher",
    subHeader1: "Claim your ",
    subHeader2: "$750",
    subHeader3: " Target voucher",
    steps: [
      "Click \"Claim Now\"",
  ...
  es: {
    ...
    header: "Cupón de Target",
    subHeader1: "Reclama tu cupón de Target de ",
    subHeader2: "$750",
    subHeader3: "",
    steps: [
    ...
    faqs: [
      ...
      },
      {
        ...
        answer: "Una vez que se verifiquen tus tareas, el código de tu cupón de Target será enviado a tu correo dentro de 24 a 48 horas."
      }
    ],
    excellent: "Excelente"
  }
      ...
      <main className="flex-1 flex flex-col items-center w-full max-w-[400px] mx-auto px-4 pt-6 pb-8">
        ...
        <div className="flex justify-center items-center gap-1.5 mb-2 w-full">
           <button 
             ...
             className={`px-3.5 py-1.5 rounded-full text-[13px] flex items-center transition-colors border ${language === 'us' ? 'bg-[#CC0000] text-white border-[#CC0000] shadow-sm' : 'bg-white text-[#CC0000] border-gray-200 shadow-sm'}`}
           >
             <span className="font-extrabold mr-1">US</span> <span className="font-medium">English</span>
           </button>
           <button 
             onClick={() => setLanguage('es')}
             className={`px-3.5 py-1.5 rounded-full text-[13px] flex items-center transition-colors border ${language === 'es' ? 'bg-[#CC0000] text-white border-[#CC0000] shadow-sm' : 'bg-white text-[#CC0000] border-gray-200 shadow-sm'}`}
           >
        ...
        <div className="h-[73px] mb-5 mt-1 flex justify-center items-center w-full">
           <img
            src="https://upload.wikimedia.org/wikipedia/commons/d/db/Target_old_logo.svg"
            alt="Target"
        ...
        <div className="w-full flex flex-col gap-2.5 mb-7">
          {t.steps.map((step, index) => (
            ...
            >
              <div className="w-8 h-8 rounded-full bg-[#CC0000] text-white border-[1.5px] border-[#CC0000] flex items-center justify-center font-bold text-[13px] shrink-0 ml-3.5 mr-3.5">
                {index + 1}
              </div>
        ...
        {/* CTA */}
        <button 
           onClick={() => window.location.href = 'https://giftclick.org/aff_c?offer_id=1381&aff_id=144760'}
